...
public class WindowsConsoleInputStream extends InputStream {
	...
	public WindowsConsoleInputStream(Charset encoderCharset) {
		this(Wincon.Companion.getINSTANCE().GetStdHandle(Wincon.STD_INPUT_HANDLE), encoderCharset);
	}
	...
	private INPUT_RECORD[] readConsoleInput() throws IOException {
		...
		if (Wincon.Companion.getINSTANCE().ReadConsoleInput(hConsoleInput, lpBuffer, lpBuffer.length, lpNumberOfEventsRead)) {
	...
	private int availableConsoleInput() {
		...
		if (Wincon.Companion.getINSTANCE().GetNumberOfConsoleInputEvents(hConsoleInput, lpcNumberOfEvents)) {
