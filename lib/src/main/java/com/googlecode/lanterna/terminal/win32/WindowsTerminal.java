...
public class WindowsTerminal extends UnixLikeTerminal {
	...
	protected void acquire() throws IOException {
		...
		Wincon.Companion.getINSTANCE().SetConsoleMode(CONSOLE_OUTPUT.getHandle(), terminalOutputMode);
		...
		Wincon.Companion.getINSTANCE().SetConsoleMode(CONSOLE_INPUT.getHandle(), terminalInputMode);
	}
	...
	public void restoreTerminalSettings() {
		if (settings != null) {
			Wincon.Companion.getINSTANCE().SetConsoleMode(CONSOLE_INPUT.getHandle(), settings[0]);
			Wincon.Companion.getINSTANCE().SetConsoleMode(CONSOLE_OUTPUT.getHandle(), settings[1]);
		}
	}
	...
	public void keyEchoEnabled(boolean enabled) {
		...
		Wincon.Companion.getINSTANCE().SetConsoleMode(CONSOLE_INPUT.getHandle(), mode);
	}
	...
	protected TerminalSize findTerminalSize() {
		...
		Wincon.Companion.getINSTANCE().GetConsoleScreenBufferInfo(CONSOLE_OUTPUT.getHandle(), screenBufferInfo);
	...
	private int getConsoleInputMode() {
		...
		Wincon.Companion.getINSTANCE().GetConsoleMode(CONSOLE_INPUT.getHandle(), lpMode);
		return lpMode.getValue();
	}

	private int getConsoleOutputMode() {
		IntByReference lpMode = new IntByReference();
		Wincon.Companion.getINSTANCE().GetConsoleMode(CONSOLE_OUTPUT.getHandle(), lpMode);
		return lpMode.getValue();
	}
